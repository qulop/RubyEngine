#pragma once

#include <types/TypeTraits.hpp>



namespace Kiwi {
    struct Version {
    public:
        u32 versionFlags = 0;

    public:
        Version() = default;
        Version(u8 major, u8 minor, u8 patch) :
            versionFlags(((u32)major << 22) | ((u32)minor << 12) | ((u32)patch))
        {}

    public:
        KIWI_NODISCARD KIWI_FORCEINLINE u32 GetMajor() const {
            return versionFlags >> 22;
        }

        KIWI_NODISCARD KIWI_FORCEINLINE u32 GetMinor() const {
            return (versionFlags >> 12) & 0x3FF;
        }

        KIWI_NODISCARD KIWI_FORCEINLINE u32 GetPatch() const {
            return versionFlags & 0xFFF;
        }
    };
}