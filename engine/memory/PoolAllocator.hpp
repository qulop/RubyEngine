...
namespace Ruby::Memory {
    ...
    public:
        PoolAllocator(size_t chunkSize=POOL_ALLOC_CHUNK_SIZE_DEFAULT, size_t numOfChunks=POOL_ALLOC_NUM_OF_CHUNKS_DEFAULT);

        PoolAllocator(const PoolAllocator& other);
        PoolAllocator(PoolAllocator&& other);

        AllocatedBlock Allocate(size_t n) override;
